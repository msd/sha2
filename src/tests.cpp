...
TEST_CASE("extended int addition") { REQUIRE_NOTHROW(test_addition_commutativity()); }
...
TEST_CASE("conversion of integer types to big endian", "[to_big_endian_array][big_endian][endian]")
{
    SECTION("convert 2 byte int", "converting 0x3412 to big endian")
    {
        ...
        array<byte, 2> got = msd::utils::endian::containers::to_big_endian_array(x);
    ...
    SECTION("convert 4 byte int", "converting 0x78563412 to big endian")
    {
        ...
        array<byte, 4> got = msd::utils::endian::containers::to_big_endian_array(x);
        REQUIRE(expected == got);
    }
    ...
    SECTION("convert 8 byte int", "converting 0xf0debc9a78563412 to big endian")
    {
        ...
        array<byte, 8> expected = {byte{0x12}, byte{0x34}, byte{0x56}, byte{0x78},
                                   byte{0x9a}, byte{0xbc}, byte{0xde}, byte{0xf0}};
        array<byte, 8> got = msd::utils::endian::containers::to_big_endian_array(x);
        REQUIRE(expected == got);
    }
}
TEST_CASE("conversion of integer types to little endian",
          "[to_little_endian_array][little_endian][endian]")
{
    SECTION("convert 2 byte integer"
            "converting 0x3412 to little endian")
    {
        uint16_t x = 0x3412u;
        array<byte, 2> expected = {byte{0x12}, byte{0x34}};
        array<byte, 2> got = msd::utils::endian::containers::to_little_endian_array(x);
        REQUIRE(expected == got);
    }

    SECTION("convert 4 byte integer"
            "converting 0x78563412 to little endian")
    {

        uint32_t x = 0x78563412;
        array<byte, 4> expected = {byte{0x12}, byte{0x34}, byte{0x56}, byte{0x78}};
        array<byte, 4> got = msd::utils::endian::containers::to_little_endian_array(x);
        REQUIRE(expected == got);
    }

    SECTION("convert 8 byte integer"
            "converting 0xf0debc9a78563412 to little endian")
    {
        uint64_t x = 0xf0debc9a78563412ull;
        array<byte, 8> expected = {byte{0x12}, byte{0x34}, byte{0x56}, byte{0x78},
                                   byte{0x9a}, byte{0xbc}, byte{0xde}, byte{0xf0}};
        array<byte, 8> got = msd::utils::endian::containers::to_little_endian_array(x);
        REQUIRE(expected == got);
    }
}

TEST_CASE("conversion of little endian bytes to integer types",
          "[from_little_endian_array][little_endian][endian]")
{
    SECTION("convert  little endian bytes to integer type",
            "converting bytes from little endian to 0x3412")
    {
        array<byte, 2> x = {byte{0x12}, byte{0x34}};
        uint16_t expected = 0x3412u;
        auto got = msd::utils::endian::containers::from_little_endian_array<uint16_t>(x);
        REQUIRE(expected == got);
    }

    SECTION("convert  little endian bytes to integer type",
            "converting bytes from little endian to 0x78563412")
    {
        array<byte, 4> x = {byte{0x12}, byte{0x34}, byte{0x56}, byte{0x78}};
        uint32_t expected = 0x78563412;
        auto got = msd::utils::endian::containers::from_little_endian_array<uint32_t>(x);
        REQUIRE(expected == got);
    }

    SECTION("convert  little endian bytes to integer type",
            "converting bytes from little endian to 0xf0debc9a78563412")
    {
        array<byte, 8> x = {byte{0x12}, byte{0x34}, byte{0x56}, byte{0x78},
                            byte{0x9a}, byte{0xbc}, byte{0xde}, byte{0xf0}};
        uint64_t expected = 0xf0debc9a78563412ull;
        auto got = msd::utils::endian::containers::from_little_endian_array<uint64_t>(x);
        REQUIRE(expected == got);
    }
}

TEST_CASE("conversion of big endian bytes to integer types",
          "[from_big_endian_array][big_endian][endian]")
{
    SECTION("convert  big endian bytes to integer type",
            "converting bytes from little endian to 0x3412")
    {
        array<byte, 2> x = {byte{0x12}, byte{0x34}};
        uint16_t expected = 0x1234u;
        auto got = msd::utils::endian::containers::from_big_endian_array<uint16_t>(x);
        REQUIRE(expected == got);
    }
            ...
            "converting bytes from little endian to 0x78563412")
    {
        ...
        auto got = msd::utils::endian::containers::from_big_endian_array<uint32_t>(x);
        REQUIRE(expected == got);
    }
            ...
            "converting bytes from little endian to 0xf0debc9a78563412")
    {
        ...
        auto got = msd::utils::endian::containers::from_big_endian_array<uint64_t>(x);
        REQUIRE(expected == got);
    }
}
...
//     {
//         vector<byte> message{byte(0x10), byte(0x20), byte(0x30), byte(0x40)};
...
//         auto block_pad_zeros =
//             ranges::subrange(cend(block_message) + 1, cbegin(padded) + BLOCK_ROLLOVER_BYTES);
